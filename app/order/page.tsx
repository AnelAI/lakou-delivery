...
import {
  Phone, MapPin, Package, Send, Navigation, CheckCircle,
  Clock, ChevronRight, Bike,
} from "lucide-react";
...
export default function OrderPage() {
  ...
  const setPickup = (loc: { name: string; lat: number; lng: number }) =>
    setForm((f) => ({ ...f, pickupAddress: loc.name, pickupLat: loc.lat.toString(), pickupLng: loc.lng.toString() }));

  const setDelivery = (loc: { name: string; lat: number; lng: number }) =>
    setForm((f) => ({ ...f, deliveryAddress: loc.name, deliveryLat: loc.lat.toString(), deliveryLng: loc.lng.toString() }));
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        setForm((f) => ({
          ...
          deliveryLat: pos.coords.latitude.toString(),
          deliveryLng: pos.coords.longitude.toString(),
        }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.pickupLat || !form.deliveryLat) {
      setError("Veuillez sélectionner les adresses");
      return;
    }
      ...
      <div className="min-h-screen bg-gradient-to-br from-orange-50 to-amber-50 flex flex-col items-center justify-center p-6">
        <div className="bg-white rounded-3xl shadow-xl p-8 w-full max-w-sm text-center">
          ...
          <p className="text-gray-500 text-sm mb-6">
            Votre commande a été reçue. Nous vous contacterons bientôt.
          </p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-orange-50 via-amber-50 to-yellow-50">
      {/* Header */}
      <div className="bg-gradient-to-r from-orange-500 to-amber-500 px-6 pt-12 pb-8 text-white">
        <div className="max-w-sm mx-auto">
          ...
          {/* Call button — prominently visible */}
          <a
            href={`tel:${MANAGER_PHONE}`}
            className="mt-4 flex items-center justify-center gap-3 w-full bg-white text-orange-600 py-4 rounded-2xl font-bold text-lg shadow-lg hover:bg-orange-50 active:scale-95 transition-all"
          >
            <Phone size={22} className="animate-pulse" />
            Appeler Motaz maintenant
          </a>
      ...
      <div className="max-w-sm mx-auto px-4 py-6">
        <div className="flex items-center gap-3 mb-6">
          ...
          <span className="text-xs text-gray-400 font-medium">OU COMMANDER EN LIGNE</span>
          <div className="h-px flex-1 bg-gray-200" />
        </div>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-purple-100 space-y-3">
            ...
            {form.pickupAddress && (
              <div className="flex items-center gap-1 text-xs text-purple-600">
                <MapPin size={11} />
                {form.pickupAddress} — coordonnées enregistrées
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 shadow-sm border border-orange-100 space-y-3">
            ...
            <button
              ...
              className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl border-2 border-dashed text-sm font-medium transition-all
                ${form.deliveryLat && form.deliveryAddress === "Ma position GPS"
            ...
            >
              <Navigation
                size={16}
                className={gpsLoading ? "animate-spin" : form.deliveryLat && form.deliveryAddress === "Ma position GPS" ? "text-green-500" : ""}
              />
              {gpsLoading
